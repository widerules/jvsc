...
public class TenoriView extends View
{
	...
	private final Paint						mUnfillPaint	= new Paint();
	private final Paint						mBrigthPaint	= new Paint();
	...
	public TenoriView(Context context, AttributeSet attrs, int defStyle)
	{
		...
		mUnfillPaint.setColor(Color.argb(128, 40, 40, 40));
		mFillPaint.setColor(Color.argb(204, 120, 120, 120));
		mBrigthPaint.setColor(Color.argb(255, 255, 255, 255));
 	...
 	private void paintRow(Canvas canvas )
 	{
 		...
 		for(int i = row; i < length; i+= flapLength)
 		{
 			...
 			if(padswitches[e.index])
 			{
 				if(rounded)
 		 		{
 					canvas.drawRoundRect(e.gridRect, 5, 5, mBrigthPaint);
 		 		}
 				else
 				{
 					canvas.drawRect(e.gridRect, mBrigthPaint);
 				}
 	...
 	private void paintOneGrid(Canvas canvas, TenoriElement tenori)
 	{
 		...
 		if(rounded)
 		{
 	 		if(padswitches[tenori.index])
	 			...
	 			canvas.drawRoundRect(tenori.gridRect, 5, 5, mUnfillPaint);
 		...
 		else
 		{
	 		if(padswitches[tenori.index])
	 			...
	 			canvas.drawRect(tenori.gridRect, mUnfillPaint);
				...
				for(WaveElement w : waves)
				{
					...
					{					
						...
						if(color)
						{
							 ...
							 {
									// Palette style
									r = (int) (255 - 42.5 * e.index_x);
									g = (int) (255 - 42.5 * e.index_y);
									b = (int) (w.t * 5);
						...
						{
							r = g= b= random.nextInt((int)d * 100);
						}
