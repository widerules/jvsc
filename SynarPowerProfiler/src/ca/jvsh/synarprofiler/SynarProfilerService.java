...
public class SynarProfilerService extends Service
{
	...
	//CPU power files
	private File					mCpu0FreqFile;
	private File					mCpu0VoltageFile;
	private File					mCpu0CurrentFile;
	private File					mCpu1FreqFile;
	private File					mCpu1VoltageFile;
	private File					mCpu1CurrentFile;

	private TFloatList				mCpu0FreqList;
	private TFloatList				mCpu0VoltageList;
	private TFloatList				mCpu0CurrentList;
	private TFloatList				mCpu1FreqList;
	private TFloatList				mCpu1VoltageList;
	private TFloatList				mCpu1CurrentList;
	...
	private boolean					mTrackCPULoad;

	private File					mCpuStatFile;

	private TFloatList				mCpu0UserList;
	private TFloatList				mCpu0SystemList;
	private TFloatList				mCpu0IdleList;
	private TFloatList				mCpu0NiceList;
	private TFloatList				mCpu0IoList;

	private TFloatList				mCpu1UserList;
	private TFloatList				mCpu1SystemList;
	private TFloatList				mCpu1IdleList;
	private TFloatList				mCpu1NiceList;
	private TFloatList				mCpu1IoList;

	private float					mUserLoad;
	private float					mUserSystem;
	private float					mUserIdle;
	private float					mUserNice;
	private float					mUserIo;
	...
	public void onCreate()
	{
		...
		try
		{
			...
			mTrackCPULoad = mSettings.getBoolean("cpu_load", false);
			if (mTrackCPULoad)
			{
				mOutput.write("Cpu0 User, Cpu0 System, Cpu0 Idle, Cpu0 Nice, Cpu0 io, Cpu1 User, Cpu1 System, Cpu1 Idle, Cpu1 Nice, Cpu1 io, ");
				mCpuStatFile = new File("/proc/stat");
				
				mCpu0UserList = new TFloatArrayList();
				mCpu0SystemList = new TFloatArrayList();
				mCpu0IdleList = new TFloatArrayList();
				mCpu0NiceList = new TFloatArrayList();
				mCpu0IoList = new TFloatArrayList();
			
				mCpu1UserList = new TFloatArrayList();
				mCpu1SystemList = new TFloatArrayList();
				mCpu1IdleList = new TFloatArrayList();
				mCpu1NiceList = new TFloatArrayList();
				mCpu1IoList = new TFloatArrayList();
			}
				...
				while (mActive)
				{
					try
					{
						...
						if( mTrackCPULoad )
						{
							...
							lines = line.split("\\s+");
							mUserLoad = Integer.valueOf(lines[1]).intValue();
							mUserSystem = Integer.valueOf(lines[3]).intValue();
							mUserIdle = Integer.valueOf(lines[4]).intValue();
							mUserNice = Integer.valueOf(lines[2]).intValue();
							mUserIo = Integer.valueOf(lines[5]).intValue();
							
							mTotal = mUserLoad + mUserSystem + mUserIdle+mUserNice + mUserIo;
							mCpu0UserList.add(mUserLoad/mTotal);
							mCpu0UserList.add(mUserSystem/mTotal);
							mCpu0UserList.add(mUserIdle/mTotal);
							mCpu0UserList.add(mUserNice/mTotal);
							mCpu0UserList.add(mUserIo/mTotal);
							...
							mTotal = mUserLoad + mUserSystem + mUserIdle+mUserNice + mUserIo;
							mCpu1UserList.add(mUserLoad/mTotal);
							mCpu1UserList.add(mUserSystem/mTotal);
							mCpu1UserList.add(mUserIdle/mTotal);
							mCpu1UserList.add(mUserNice/mTotal);
							mCpu1UserList.add(mUserIo/mTotal);
		...
		try
		{
			for (int i = 0; i < mPowerSensorsValues; i++)
			{
				...
				if( mTrackCPULoad )
				{
					...
					mOutput.write(mCpu0UserList.get(i) + ", ");
					
					mOutput.write(mCpu0SystemList.get(i) + ", ");
					
					mOutput.write( mCpu0IdleList.get(i) + ", ");
					
					mOutput.write(mCpu0NiceList.get(i) + ", ");
					
					mOutput.write(mCpu0IoList.get(i) + ", ");
					
					
					
					mOutput.write(mCpu1UserList.get(i) + ", ");
					
					mOutput.write(mCpu1SystemList.get(i) + ", ");
					
					mOutput.write( mCpu1IdleList.get(i) + ", ");
					
					mOutput.write(mCpu1NiceList.get(i) + ", ");
					
					mOutput.write(mCpu1IoList.get(i) + ", ");
				}
			...
			{
				mCpu0UserList.clear();
				mCpu0SystemList.clear();
				mCpu0IdleList.clear();
				mCpu0NiceList.clear();
				mCpu0IoList.clear();
				
				mCpu1UserList.clear();
				mCpu1SystemList.clear();
				mCpu1IdleList.clear();
				mCpu1NiceList.clear();
				mCpu1IoList.clear();
			}
